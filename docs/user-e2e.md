...
On the **Preview Ranking Lab** fixture, smoke asserts panel/export parity for the scan-level too-short callout (`preview_ranking_scan_callout=`), the selected-song header (`preview_ranking_selected_header=`), and per-song too-short breakdown lines (`too_short_song=`):
