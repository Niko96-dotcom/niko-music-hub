...
|---|---|---|---|---|---|
| F1 | Medium | `DiagnosticsPathRedactor` | `redact` and `redactPathsInText` matched the home directory by plain string prefix, so a sibling account named `nikolaus` (home account `niko`) was rewritten to the wrong `~/laus/...` path in exports and logs. | Match only the home directory itself or paths below it (`/` boundary). | `DiagnosticsPathRedactorTests` (2 new cases) |
