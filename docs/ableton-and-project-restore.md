...
**Archive Now** verifies the Vault generation and current project, checks that the DAW and project files are closed, then removes the Active copy. The song leaves the normal board and becomes a **Restore & Open** target under **Show archived projects**. A restored, unchanged song can reuse its verified generation when archived again.
...
For a guided path, use the recovery copy in **Settings → Project Vault → Recovery copy** (available even with Project Vault disabled): **Export recovery metadata** snapshots the catalog and settings while holding the Vault mutation lock, then **Open recovered library** validates a copy and stages it as a NEW isolated library that launches as a separate app instance with automation disabled and paused. The copy holds metadata only — Vault project files, plug-ins, and samples still need their own backups (Cubase **File → Backup Project**, Ableton **File → Collect All and Save**). Keep the damaged original; a missing backup catalog cannot be reconstructed, and preserved generations remain for manual DAW recovery with no deletion. Same-Mac recovery is the supported and tested boundary; replacement-Mac is not proven. Full steps: [Project Vault recovery copies](vault-recovery.md).
