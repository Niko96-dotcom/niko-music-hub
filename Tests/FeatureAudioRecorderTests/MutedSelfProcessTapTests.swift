...
final class MutedSelfProcessTapTests: XCTestCase {
    func testStuckAggregateCreationDoesNotKeepTheAppMutedPastTheDeadline() async throws {
        let hal = FakeTapHAL(hangAggregateCreation: true)
        defer { hal.releaseAggregateCreation() }
        ...
        XCTAssertTrue(hal.aggregateCreationPending, "the start call is still stuck")
    ...
    func testHungProcessLookupRunsOnTheTapQueueAndNeverCreatesALateTap() async throws {
        let hal = FakeTapHAL(hangAggregateCreation: false)
...
/// app, exactly like `CATapMuteBehavior.muted`; destroying it un-mutes.
final class FakeTapHAL: @unchecked Sendable {
    ...
    private let lock = NSLock()
    private let hangAggregateCreation: Bool
    private let aggregateGate = DispatchSemaphore(value: 0)
    ...
    init(hangAggregateCreation: Bool) {
        self.hangAggregateCreation = hangAggregateCreation
    }
    ...
    var aggregateCreationPending: Bool { lock.withLock { pending } }

    func releaseAggregateCreation() {
        let signal = lock.withLock { () -> Bool in
            defer { released = true }
            return !released
        }
        if signal { aggregateGate.signal() }
    }
    ...
    var hal: SystemAudioTapHAL {
        SystemAudioTapHAL(
            ...
            readDefaultSystemOutputDevice: { (AudioObjectID(0x7FFF_FF12), "fake-output") },
            createAggregateDevice: { [self] _ in
                if hangAggregateCreation {
